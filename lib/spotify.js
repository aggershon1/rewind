...
// Reuses the same fuzzy artist-name check as searchArtistTracks, so a track only
// counts as a match if one of its credited artists is actually the one requested.
function artistNameMatches(trackArtists, expectedArtist) {
  if (!expectedArtist) return true;
  const normalizedExpected = expectedArtist.trim().toLowerCase();
  return trackArtists.some((a) => {
    const n = a.name.trim().toLowerCase();
    return n === normalizedExpected || n.includes(normalizedExpected) || normalizedExpected.includes(n);
  });
}

// Resolves one AI recommendation ({ type, name, artist, genre, reason }) to a real
// Spotify track via search. Checks several candidates against the expected artist name
// rather than trusting the single top hit — a bare word-match search can otherwise
// return a completely unrelated track when the exact title/artist isn't on Spotify
// (e.g. an obscure or slightly-misremembered pick), and returning that would be worse
// than returning nothing.
async function searchTrackUri(token, rec) {
  const tryQuery = async (q) => {
    const data = await spotifyFetch(token, `/search?q=${encodeURIComponent(q)}&type=track&limit=5`);
    return data?.tracks?.items || [];
  };

  const expectedArtist = rec.type === 'track' ? rec.artist : rec.name;
  ...
  let candidates = await tryQuery(preciseQuery).catch(() => []);
  let track = candidates.find((t) => artistNameMatches(t.artists, expectedArtist)) || null;

  if (!track) {
    const looseQuery = rec.type === 'track' ? `${rec.name} ${rec.artist || ''}` : rec.name;
    candidates = await tryQuery(looseQuery).catch(() => []);
    track = candidates.find((t) => artistNameMatches(t.artists, expectedArtist)) || null;
  }

  if (!track) return null; // no confident match — better to show nothing than the wrong song
...
async function searchArtistTracks(token, artistName, excludeTrackName, limit = 5) {
  ...
  for (const t of items) {
    if (!artistNameMatches(t.artists, artistName)) continue;
...
// Generic cursor-following paginator for endpoints that return a `next` field with a
// full URL to the following page (playlists, saved tracks, playlist items). Capped at
// maxPages as a safety net for very large libraries — a personal tool doesn't need to
// walk someone's entire 20,000-track Liked Songs list on every call.
async function paginate(token, initialPath, extractItems, { maxPages = 40 } = {}) {
  let results = [];
  let nextUrl = `https://api.spotify.com/v1${initialPath}`;
  let pages = 0;
  ...
  while (nextUrl && pages < maxPages) {
    ...
    if (!resp.ok) {
      const text = await resp.text();
      throw Object.assign(new Error(`Spotify API error (${resp.status}) on ${nextUrl}: ${text}`), {
        status: resp.status,
      });
    }
    const data = await resp.json();
  ...
  return { items: results, truncated: !!nextUrl };
}

async function getUserPlaylists(token) {
  const { items } = await paginate(token, '/me/playlists?limit=50', (d) => d.items || []);
  return items.map((p) => ({
    id: p.id,
    name: p.name,
...
async function getPlaylistTrackRefs(token, playlistId) {
  const { items } = await paginate(token, `/playlists/${playlistId}/items?limit=100`, (d) => d.items || []);
  return items
...
async function getSavedTrackRefs(token) {
  const { items } = await paginate(token, '/me/tracks?limit=50', (d) => d.items || []);
  return items
    .filter((it) => it.track && it.track.id)
    .map((it) => ({
      id: it.track.id,
