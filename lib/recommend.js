...
function buildPrompt(history, likedFeedback = [], dislikedFeedback = [], duplicateFeedback = [], weights = {}, count = 10) {
...
Aim for ${count} total picks. If the one-pick-per-artist rule and genuine stylistic fit mean you can't honestly reach ${count} distinct, well-justified artists, return fewer rather than padding with weak or repetitive picks — quality and fit matter more than hitting the exact number.

Respond with ONLY a JSON array (no markdown fences, no commentary), each item shaped like:
...
async function getRecommendations(history, likedFeedback = [], dislikedFeedback = [], duplicateFeedback = [], weights = {}, count = 10) {
  const prompt = buildPrompt(history, likedFeedback, dislikedFeedback, duplicateFeedback, weights, count);
